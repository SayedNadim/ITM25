...
def select_dataset(data_dir, mode):
    ...
    if mode == "test":
        ...
        hdr_pattern = os.path.join(data_dir, "test/hdr/*.hdr")
        hdr_files = sorted(glob.glob(hdr_pattern))
        path = []
        for hdr_file in hdr_files:
            # Get the base filename (e.g., "1" from "1.hdr")
            base_name = os.path.splitext(os.path.basename(hdr_file))[0]
            # Construct corresponding LDR file path
        ...
        # hdr_files = sorted(glob.glob(hdr_pattern))
        hdr_files = sorted(glob.glob(os.path.join(data_dir, "valid/hdr/*.hdr")))
        path = []
        for hdr_file in hdr_files:
            base_name = os.path.splitext(os.path.basename(hdr_file))[0]
            # Construct corresponding LDR file path
            ldr_file = os.path.join(data_dir, "valid/ldr", f"{base_name}.jpg")
            if os.path.exists(ldr_file):
                path.append((ldr_file, hdr_file))  # (low_res, high_res)
    else:
        raise NotImplementedError(f"{mode} is not implemented in select_dataset")
